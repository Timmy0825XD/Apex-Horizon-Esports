...
`/bot ping` y `a?ping` son el mismo diagnóstico. El prefijo de texto es `a?`. Slash: comando `bot`, subcomandos `ping`, `about`, `help`.
...
**Para qué:** Comprobar que la sesión WebSocket, la respuesta a la interacción (o al mensaje) y el almacén están vivos. También muestra RAM, uptime y alcance (servidores / miembros).
...
**Para qué:** Mapa de comandos por sección y quién puede usarlos. El heading de cada página es el comando raíz de esa familia. Los de `/bot`, `/settings` y `/server` se mencionan como slash; el resto del catálogo va en monospace hasta que existan en Discord.
