...
Menciones de slash commands: `</name:id>` vía `formatHelpEntry` (en `/bot help` para subcomandos de `bot`, `settings`, `server`, `staff`, `tournament`, `team` y `utility`; el resto del catálogo usa `` `/comando` `` hasta estar registrados).
